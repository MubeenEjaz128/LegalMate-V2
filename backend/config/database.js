 ...
 * Never logs database credentials or connection strings.
 */
const connectDB = async () => {
  const uri = process.env.MONGODB_URI || 'mongodb://localhost:27017/legalmate'
  if (!process.env.MONGODB_URI) {
    console.warn('[LegalMate] Warning: MONGODB_URI not set in environment. Using default local MongoDB instance.')
  }
  try {
