...
// // Get user profile by ID (public endpoint with limited info)
...
//   try {
//     console.log('Getting authenticated user profile:', req.user.userId);
...
//     if (user.role !== 'client') {
//       return res.status(403).json({ message: 'Credit balance only available for clients.' });
//     }
//   const userId = typeof req.user.userId === 'string' ? new (require('mongoose').Types.ObjectId)(req.user.userId) : req.user.userId;
//     let creditDoc = await Credit.findOne({ user: userId });
//     if (!creditDoc) {
//       creditDoc = await Credit.create({ user: userId, balance: 0 });
//     }
...
//     // Add rating for lawyers (placeholder)
...
//       'name', 'phone', 'address', 'bio', 'isAvailable', 'languages', 'hourlyRate', 'specialization', 'barNumber',
//       'social', 'profileVisibility', 'status'
//     ];

//     const updates = {};
//     allowedUpdates.forEach(field => {
//       if (req.body[field] !== undefined) {
//         // Handle languages array sent as 'languages[]' from FormData
//         if (field === 'languages' && req.body['languages[]']) {
//           const langArray = Array.isArray(req.body['languages[]']) ? req.body['languages[]'] : [req.body['languages[]']];
//           updates[field] = langArray;
//           return;
//         }

//         // Parse JSON fields if sent as string (from FormData)
//         if (['languages', 'social'].includes(field) && typeof req.body[field] === 'string') {
//           try { 
...
//           if (field === 'hourlyRate') {
//             const rate = parseFloat(req.body[field]);
...
//       updates.profilePicture = req.files.profilePictureFile[0].path.replace(/\\/g, '/').replace(/^.*uploads\//, 'uploads/');
//     }

//     // Handle document file upload (if needed)
//     if (req.files && req.files.documentFile) {
//       // Add document handling logic here if needed
//       console.log('Document file uploaded:', req.files.documentFile[0].filename);
//     }
...
router.post('/login', [
  ...
  try {
    ...
    // Single-device enforcement: block login if actively used on another device
    // A session is considered "active" if heartbeat was received within last 2 minutes
    const HEARTBEAT_THRESHOLD = 2 * 60 * 1000; // 2 minutes
    if (
      user.activeSessionToken &&
      user.lastHeartbeat &&
      (Date.now() - new Date(user.lastHeartbeat).getTime()) < HEARTBEAT_THRESHOLD
    ) {
      console.log('⛔ Already logged in on another device (recent heartbeat):', email)
      return res.status(409).json({
        message: 'This account is currently active on another device. Please logout from that device first, or wait a few minutes.',
